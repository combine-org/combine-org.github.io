...
* **Creating new event**:
  1) Copy Harmony-2021, and fill up the template
  2) add "Events" tag

* **Changing current event to past event**:
  1) Author update tag from "Events" to "Past Events"
  2) Home/current.md and change author to new current event. If there is NO new current (or comment out entire file)
  3) Events/current.md change author to new event (Always have to have new event). If there is NO new current (or comment out entire file)
